...
class ImagenViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBOutlet weak var elegirbtn: UIButton!
    override func viewDidLoad() {
        super.viewDidLoad()
        imagePicker.delegate = self
        elegirbtn.isEnabled = false
    ...
    @IBAction func elegirContacto(_ sender: Any) {
        self.elegirbtn.isEnabled = false
        ...
        let cargarImagen = imagenesFolder.child("\(NSUUID().uuidString).jpg").putData(imagenData!, metadata: nil) { (metadata, error) in
            if error != nil {
                self.mostarAlerta(titulo: "Error", mensaje: "Se produjo un error al subir la imagen. Compruebe su conexion a internet", accion: "Aceptar")
                self.elegirbtn.isEnabled = true
